...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if ([EMVoiceConverter isMP3File:aFilePath]) {
        ...
        if (![fileManager fileExistsAtPath:wavFilePath]) {
            ...
            if (!covertRet) {
                if (completon) {
                    completon([NSError errorWithDomain:NSLocalizedString(@"im.FileFormatConversionFailed", nil)
...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"im.RecordingisnotOver", nil)
    ...
    if (!fileName || [fileName length] == 0) {
        error = [NSError errorWithDomain:NSLocalizedString(@"im.FilePathDoesNotExist", nil)
                                    code:-1
                                userInfo:nil];
        completion(error);
        return ;
...
-(void)asyncStopRecordingWithCompletion:(void(^)(NSString *recordPath,
    ...
    if(![self isRecording]){
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"im.Haven'tStartedvoicerecording", nil)
    ...
    if([_recorderEndDate timeIntervalSinceDate:_recorderStartDate] < [EMCDDeviceManager recordMinDuration]){
        if (completion) {
            error = [NSError errorWithDomain:NSLocalizedString(@"im.Recording time is too short", nil)
    ...
    if (isNeedActive) {
        BOOL success = [audioSession setActive:isActive
                                   withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation
                                         error:&error];
        if(!success || error){
            error = [NSError errorWithDomain:NSLocalizedString(@"im.FailedToInitializeAVAudioPlayer!", nil)
                                        code:-1
                                    userInfo:nil];
            return error;
        }
