...
- (void)joinChatroom:(NSString *)chatroomId
{
    ...
    [self showHudInView:self.view hint:NSLocalizedString(@"im.joining", nil)];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (weakSelf) {
                ...
                [strongSelf hideHud];
                if (error != nil) {
                    [strongSelf showHint:[NSString stringWithFormat:NSLocalizedString(@"im.joinRoomFail", nil), chatroomId]];
                } else {
                            ...
                            username:(NSString *)aUsername
{
    ...
    [self showHint:[NSString stringWithFormat:NSLocalizedString(@"im.joinChat", nil), aUsername, aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}
...
{
    CGRect frame = self.chatToolbar.frame;
    [self showHint:[NSString stringWithFormat:NSLocalizedString(@"im.LeaveChatRoom", nil), aUsername, aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
}
                        ...
                        reason:(EMChatroomBeKickedReason)aReason
{
    if ([_conversation.conversationId isEqualToString:aChatroom.chatroomId])
    {
        ...
        if (aReason == EMChatroomBeKickedReasonOffline) {
            UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"(ಥ_ಥ)" message:[NSString stringWithFormat:NSLocalizedString(@"im.rejoinTheChatRoom", nil), aChatroom.chatroomId] preferredStyle:UIAlertControllerStyleAlert];
            ...
            CGRect frame = self.chatToolbar.frame;
            [self showHint:[NSString stringWithFormat:NSLocalizedString(@"im.kickedOutChatRoom", nil), aChatroom.chatroomId] yOffset:-frame.size.height + KHintAdjustY];
                   ...
                   messageType:(EMMessageBodyType)messageType
{
    ...
    if (_deleteMenuItem == nil) {
        _deleteMenuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"delete", nil) action:@selector(deleteMenuAction:)];
    }
    
    if (_copyMenuItem == nil) {
        _copyMenuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"copy", nil) action:@selector(copyMenuAction:)];
    }
...
- (void)_downloadMessageAttachments:(EMMessage *)message
{
    ...
    void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage, EMError *error) {
        ...
        {
            [weakSelf showHint:NSLocalizedString(@"im.failedToGetThumbnails", nil)];
        }
    };
                ...
                if (isAutoDownloadThumbnail) {
                    [[EMClient sharedClient].chatManager downloadMessageAttachment:message progress:nil completion:^(EMMessage *message, EMError *error) {
                        if (!error) {
                            ...
                            [weakSelf showHint:NSLocalizedString(@"im.getVoiceFailed", nil)];
                        }
                    }];
                }
            }
...
- (void)_videoMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if ([localPath length] == 0) {
        [self showHint:NSLocalizedString(@"im.FailedToGetVideo", nil)];
        return;
    }
    ...
    [self showHudInView:self.view hint:NSLocalizedString(@"im.gettingVideo...", nil)];
    if (isCustomDownload) {
        [self _customDownloadMessageFile:model.message];
    } else {
        [[EMClient sharedClient].chatManager downloadMessageAttachment:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
            ...
            if (!error) {
                ...
                [weakSelf showHint:NSLocalizedString(@"im.FailedToGetVideo", nil)];
            }
        }];
    }
}
    ...
    if ([imageBody type] == EMMessageBodyTypeImage) {
        if (imageBody.thumbnailDownloadStatus == EMDownloadStatusSuccessed) {
            ...
            void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage, EMError *error) {
                ...
                [weakSelf showHint:NSLocalizedString(@"im.failedToGetBigPicture", nil)];
            };
...
- (void)_audioMessageCellSelected:(id<IMessageModel>)model
{
    ...
    if (downloadStatus == EMDownloadStatusDownloading) {
        [self showHint:NSLocalizedString(@"im.downloadingVoice", nil)];
        return;
    }
                    ...
                    if (asset) {
                        [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData *data, NSString *uti, UIImageOrientation orientation, NSDictionary *dic){
                            if (data != nil) {
                                ...
                                [self showHint:NSLocalizedString(@"im.pictureIsTooBig", nil)];
                            }
                        }];
                    }
                }];
    ...
    for (EMMessage *message in aCmdMessages) {
        if ([self.conversation.conversationId isEqualToString:message.conversationId]) {
            ...
            if ([body.action isEqualToString:@"TypingBegin"]) {
                self.title = NSLocalizedString(@"im.Typing", nil);
                continue;
            ...
            [self showHint:NSLocalizedString(@"im.TransparentMessage", nil)];
        }
    }
}
...
-(NSDictionary *)_getExtDict{
    Class userClass = NSClassFromString(@"DataContext");
    NSDictionary *ext = nil;
    if ([[userClass class] respondsToSelector:@selector(getUserData)]) {
        NSDictionary *userDict = [[userClass class] performSelector:@selector(getUserData)];
        if (userDict && [userDict isKindOfClass:[NSDictionary class]]) {
            ...
            ext = @{@"nickname":nickname,@"headimg":headimg};
        }
    }
    return ext;
}
...
- (void)sendImageMessageWithData:(NSData *)imageData
{
    ...
    EMMessage *message = [EaseSDKHelper getImageMessageWithImageData:imageData to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:ext];
    ...
    if (text.length > 1) {
        ...
        if ([splits count]) {
            for (NSString *split in splits) {
                if (split.length) {
                    NSString *atALl = NSLocalizedString(@"im.AllNews", nil);
